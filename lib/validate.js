...
function validateContent(c) {
  ...
  for (const [k, v] of Object.entries(c)) {
    if (!KEY_RE.test(k) || BLOCKED_KEYS.has(k) || k === "features") continue;
    if (typeof v === "string") {
  ...
  out.features = feats.map((f, i) => {
    ...
    return {
      icon: str(f.icon, `Diferencial ${i + 1} (ícone)`, { max: 16 }),
...
function validateCategories(list) {
  ...
  return list.map((c, i) => {
    ...
    return {
      ...
      icon: str(c.icon, `Categoria ${i + 1} (ícone)`, { max: 16 }) || "🍽️",
      color,
    };
  });
}
...
function validateProducts(list, catIds) {
  ...
  return list.map((p, i) => {
    ...
    return {
      ...
      img: str(p.img, `${label} (emoji)`, { max: 16 }) || "🌮",
      photo,
